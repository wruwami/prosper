...
| Making the CFG dispatcher's entry-M0 token set PRECISE about the guest is safe, because a token that is wrongly dropped only turns a reject into a success | **Falsified in review of #3312.** Success is not the safe direction here: `load_state` gives every referenced scalar a Function-variable value, `save_state` stores **zero** for a written register with no `state.sreg` entry, and `operand_bits` consults `rs.sreg` first — so a dropped token reads back as **0 with `ok` true**, a silently fabricated word where there was a loud reject. The emitter's token deliberately survives every value-less write (VOPC SGPR destinations, the B32 mask logical family at `rdna2_emit_alu.cpp:1703-1707`, `s_getpc_b64`); its only erase is the SOP1 generic path at `:1354`, and `record_scalar_write` never touches `sreg_entry_m0`. A KILL over `for_each_scalar_write` fires on all of them, so the dataflow has **no KILL** — it is a superset of the emitter's own token lifetime by construction. A precise KILL needs a value-publishing predicate shared with `emit_alu`; approximating one is how this goes wrong. | #3308, #3312 |
| A synthetic arm can discriminate that KILL, so the guard can be pinned by a test rather than by an argument | **Falsified by four measured placements.** Write and read in one block: the block-entry set is never consulted. Write behind one arm of an if: the union join restores the token. Write unconditional with the read in a later block: the KILL does drop the token and the **B64 mask domain refuses the read anyway**. `s_getpc_b64`, to get a value-less write that is not a mask: the shell refuses `op=0x1f` at the write itself. Every accepted value-less scalar write is a mask write, and the mask domain guards those reads independently — so the hazard is reachable only through the **gap** in that MUST/equality join. `test_entry_m0_dispatcher`'s arm F is therefore a regression pin that says so in its own comment, not a discriminator. | #3308, #3312 |
