...
**What is actually open** is why the shader's IMAGE_LOAD names `s8` when user-data dword 0 holds a
buffer descriptor. Three readings survive, and they point at different files:

1. the guest declared that `sharp[0]` slot as an **8-dword T#** and what is there is a V# — a
   guest/driver-state question, and the closest thing to the "claimed by the V# path" story;
2. the slot is a genuine **4-dword V#** and our SRSRC decode or per-stage user-data seeding is wrong —
   a prosper question, in a different file;
3. the op is on a path the guest never takes.

**One field separates 1 from 2, and it is already printed by an instrument we run**: `PROSPER_SHARPLOG`
reports each declared slot as `ro[N]: bits=0x… offset_dw=… size=…`, where `size=1` is a four-dword V#
and `size=0` an eight-dword T#. Get that line for `0x3013540000` slot 0 before anything else.
...
- **"The five unresolved image ops are the #305/#3137 user-data window family"** and **"a texture
  ...
  **withdrawn on 2026-09-04**, within the hour, on the `PROSPER_DYNTRACE_FAIL=1` run's own
  ...
  What remains open, and the one `PROSPER_SHARPLOG` field that decides it, is in
  § *The unresolved image ops* above. #3126, #3313.
